...
@DATASETS.register_module()
class ObjaverseXL(data.Dataset):
    def __init__(self, config):
    ...
    def __getitem__(self, idx):
        ...
        sample_pc_uid, sample_render_uid = record["save_uid_pc"], record["save_uid_orig"]
        sample_pc_path = os.path.join(self.pc_path,f"{sample_pc_uid}.npz")
        data = np.load(sample_pc_path)["arr_0"]
        ...
        picked_rendered_image_dir = os.path.join(self.rendered_image_addr,sample_render_uid)
        random_render = "{:03d}".format(random.randint(0,11))
        picked_render_path = os.path.join(picked_rendered_image_dir,f"{random_render}.png")
        image = Image.open(picked_render_path)
        image = image.convert("RGB")

        image = self.train_transform(image)

        captions = [ # not used, just making code compatible
            "a rendering of an object",
            "an image",
            "a photo of a 3D object"
        ]
        tokenized_captions = []
        for c in captions:
            tokenized_captions.append(self.tokenizer(c))
        tokenized_captions = torch.stack(tokenized_captions)



        return "", "", tokenized_captions, data, image
